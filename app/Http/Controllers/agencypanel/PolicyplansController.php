...
class PolicyplansController extends Controller
{
    ...
    public function manage() {
        $data = Policyplan::where("agenc_id", "=", Auth::id())->paginate($this->pagination);
        return view('agencypanel.managepolicyplan', compact('data'));
    }
    
    public function search(Request $request) {
        $input = $request->all();
        $qry = Policyplan::where("agenc_id", "=", Auth::id()); 
    ...
    public function add() {
        ...
        $data = array('type'=>'add', 'agenc_id'=>Auth::id(), 'policytype'=>$policytype);        
        return view('agencypanel.addpolicyplan', compact('data'));
    }
    ...
    public function save(Request $request) {
        ...
        if ($validator->fails()) {
        	$policytype = Policytype::where("status", "=", "1")->pluck('policy_type_title', 'id');
            $data = array('type'=>'add', 'input'=>$input, 'policytype'=>$policytype,'agenc_id'=>Auth::id(), 'error'=>$validator->messages());
            return view('agencypanel.addpolicyplan', compact('data'));
            exit();            
        }
        ...
        if($agency->id>0) {
            return redirect()->route('agencypanel.policyplan.manage')->with('success', 'Created successfully.');
        } else {
            return redirect()->route('agencypanel.policyplan.add')->withErrors(['Error creating record. Please try again.']);
        }
    }
    ...
    public function edit($id) {
        ...
        $data = array('type'=>'edit', 'input'=>$input, 'policytype'=>$policytype, 'agenc_id'=>Auth::id());        
	    return view('agencypanel.addpolicyplan', compact('data'));
	}
    ...
    public function update(Request $request) {
        ...
        if ($validator->fails()) { 
            ...
            $policytype = Policytype::where("status", "=", "1")->pluck('policy_type_title', 'id');
            $data = array('type'=>'Edit', 'input'=>$input, 'policytype'=>$policytype,'agenc_id'=>Auth::id(), 'error'=>$validator->messages());
            return view('agencypanel.addpolicyplan', compact('data'));
            exit();            
        }
        $update = array();
        ...
        return redirect()->route('agencypanel.policyplan.manage')->with('success', 'Updated successfully.');
    ...
    public function delete($id) {
        ...
        return redirect()->route('agencypanel.policyplan.manage')->with('success', 'Deleted successfully.');
    }
