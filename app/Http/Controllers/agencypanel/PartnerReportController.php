...
class PartnerReportController extends Controller
{
    ...
    public function manage() {
    	$channel_partner = Channel_partner::where("agenc_id", "=", Auth::id())->pluck('name', 'id');
    	$data = Lead::where("agenc_id", "=", Auth::id())->where("mark_opportunity", "=", "0")->paginate($this->pagination);
    	$input["report_type"] = 1;
    	$channel_partner_id = 0;
        return view('agencypanel.partnerreport', compact('data','channel_partner','input','channel_partner_id'));
    }
    ...
    public function search(Request $request) {
        ...
        if(trim($input["report_type"])!="" ) 
        {
        	...
        	{
        		$qry = Lead::where("agenc_id", "=", Auth::id()); 
				$qry->where([
	                ["mark_opportunity", "=", "0"],
	            ]);
