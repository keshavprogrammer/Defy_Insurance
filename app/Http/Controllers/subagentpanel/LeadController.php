...
class LeadController extends Controller
{
    ...
    public function manage() {
    	$data = Lead::where("subagent_id", "=", Auth::id())->where("mark_opportunity", "=", "0")->paginate($this->pagination);
        return view('subagentpanel.managelead', compact('data'));
    }
    ...
    public function add() {
        ...
        $data = array('type'=>'add', 'agenc_id'=>$agenc_id, 'agent_id'=>$agent_id,'subagent_id'=>Auth::id(), 'policyplan'=>$policyplan);        
        return view('subagentpanel.addlead', compact('data'));
    }
    ...
    public function save(Request $request) {
        ...
        if ($validator->fails()) {
        	...
        	$policyplan = Policyplan::where("status", "=", "1")->where("agenc_id", "=", $agenc_id)->pluck('title', 'id'); 
            $data = array('type'=>'add', 'input'=>$input, 'agenc_id'=>$agenc_id, 'agent_id'=>$agent_id,'subagent_id'=>Auth::id(), 'policyplan'=>$policyplan, 'error'=>$validator->messages());
            return view('subagentpanel.addlead', compact('data'));
            exit();            
        }
        ...
        if($agency->id>0) {
            return redirect()->route('subagentpanel.lead.manage')->with('success', 'Created successfully.');
        } else {
            return redirect()->route('subagentpanel.lead.add')->withErrors(['Error creating record. Please try again.']);
        }
    }
    ...
    public function update(Request $request) {
        ...
        print_r($lead);
        return redirect()->route('subagentpanel.lead.manage')->with('success', 'Updated successfully.');
	...
	public function delete($id) {
        ...
        return redirect()->route('subagentpanel.lead.manage')->with('success', 'Deleted successfully.');
    }
    ...
    public function leadsinglesms(Request $request) {
        ...
        if($sentResponse->sid != ''){
            ...
            return redirect()->route('subagentpanel.lead.manage')->with('success', 'SMS sent successfully.');
        }
        else{
            return redirect()->route('subagentpanel.lead.manage')->withErrors(['SMS is not sent']);
        }
    }    
